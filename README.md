...
WidgetFrame.create({
  ...
  scrollToTop: true,                             // Scroll widget into view after in-widget navigation
...
| `scrollToTop` | boolean | No | `true` | After in-widget navigation, scroll the widget into view if its top edge is above the viewport. Does not fire on the initial load. |
